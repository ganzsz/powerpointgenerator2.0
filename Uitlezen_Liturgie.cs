...
namespace PowerpointGenerater2
{
    class Uitlezen_Liturgie
    {
        ...
        //bevat pad naar sheets die rechtstreeks ingevoegd mogen gaan worden
        ...
        public void LeesLiturgie(List<String> Liturgiepad)
        {
            ...
            foreach (String Liturgie in Liturgiepad)
            {
                ...
                //probeer eerst .txt als source (dat is tekst)
                path += ".txt";
                //als dit geen geldig bestand geeft is de source waarschijnlijk een powerpoint bestand
                if (!File.Exists(path))
                {
                    path = Liturgie;
                    path += ".pptx";
